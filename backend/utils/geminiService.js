    ...
    for (const card of cards) {
      ...
      for (const line of lines) {
        if (line.startsWith("Q:")) {
          ...
          if (["hard", "medium", "easy"].includes(diff)) {
            difficulty = diff;
          }
        }
      }
 ...
 * @returns {Promise<Array<{question: string, options: Array, correntAnswer: string,explanation:string difficulty:string}>>}
 */
export const generateQuiz = async (text, numQuestions = 5) => {
  const prompt = `Generate exactly ${numQuestions} multiple choice questions from the following text.
    ...
    O3: [Option 3]
    O4: [Option 4]
    C: [Correct option - index of the correct option, first option is index zero, and so on]
    E: [Brief explanation]
...
export const chatWithContext = async (question, chunks) => {
  ...
  try {
    ...
    console.error("Gemini API error:", error);
    throw new Error("Failed to process that request");
  }
};
