...
export const reviewFlashcard = async (req, res, next) => {
  try {
    ...
    const cardIndex = flashcardSet.cards.findIndex((card) =>
...
/**
 * @desc Toggle star/favourite on a flashcard
...
export const toggleStarFlashcard = async (req, res, next) => {
  try {
    ...
    res.status(200).json({
      success: true,
      data: flashcardSet,
      message: `Flashcard ${flashcardSet.cards[cardIndex].isStarred ? "unstarred" : "starred"} successfully`,
    });
  } catch (error) {
    next(error);
  }
