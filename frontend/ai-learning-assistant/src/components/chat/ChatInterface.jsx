...
function ChatInterface() {
  ...
  const messagesEndRef = useRef(null); // end of messages ref
  ...
  useEffect(() => {
    const fetchChatHistory = async () => {
      try {
        ...
        toast.error(error.error || "Could not fetch the chat history");
      } finally {
  ...
  const handleSendMessage = async (e) => {
    ...
    setMessage("");
    setLoading(true); // try to hit Gemini with the request
    try {
      const response = await aiService.chat(documentId, userMessage.content);
      if (!response) {
        toast.error("Gemini could not generate a response at this time");
        return;
      }
      ...
      <div className="flex-1 p-6 overflow-y-auto bg-linear-to-br from-slate-50/50 via-white/50 to-slate-50/50">
        {history.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-center">
            ...
            <h3 className="text-basefont-semibold text-slate-900 mb-2">
      ...
      <div className="p-5 border-t border-slate-200/60 bg-white/80">
        ...
        >
          ...
          <button
            ...
            className="shrink-0 w-12 h-12 bg-linear-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 text-white rounded-xl flex items-center justify-center transition-all duration-200 shadow-lg shadow-emerald-500/25 disabled:opacity-50 disabled:cursor-not-allowed active:scale-95"
          >
