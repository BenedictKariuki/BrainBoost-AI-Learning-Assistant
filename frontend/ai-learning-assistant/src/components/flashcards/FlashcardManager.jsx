...
function FlashcardManager({ documentId }) {
  ...
  const fetchFlashcardSets = useCallback(async () => {
    try {
      ...
      toast.error("Could not fetch flashcard sets at this time");
    } finally {
      setLoading(false);
    }
  }, [documentId]);
  ...
  const handleGenerateFlashcards = async () => {
    ...
    try {
      ...
      toast.error(error.error || "Could not generate flashcards at this time");
    } finally {
      setGenerating(false);
    }
  };

  const handleReview = async (currentCardIndex) => {
    const currentCard = selectedSet?.cards[currentCardIndex];
    if (!currentCard) return;
    try {
      await flashcardService.reviewFlashcard(currentCard._id, currentCardIndex);
      toast.success("Flashcard reviewed");
    } catch (error) {
      toast.error(error.error || "Could not review");
    }
  };
  ...
  const handleNextCard = () => {
    if (selectedSet) {
      ...
      setCurrentCardIndex(
        (prev) => (prev + 1) % selectedSet.cards.length, // looping back
      );
    }
  };
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      toast.error(error.error || "Failed to delete flashcard");
    } finally {
      setDeleting(false);
  ...
  const handleSelectedSet = (flashcardSet) => {
  ...
  const renderFlashcardViewer = () => {
    ...
    return (
      <div className="space-y-8">
        ...
        <button
          className="inline-flex group items-center gap-2 text-sm font-medium text-slate-600 hover:text-emerald-600 transition-colors duration-200"
    ...
    if (flashcardSets.length === 0) {
      return (
        <div className="flex flex-col items-center justify-center py-16 px-6">
          <div className="inline-flex justify-center items-center w-16 h-16 rounded-2xl bg-linear-to-br from-emerald-100 to-teal-100 mb-6">
