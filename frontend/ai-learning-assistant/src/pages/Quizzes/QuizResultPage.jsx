...
function QuizResultPage() {
  ...
  useEffect(() => {
    const fetchResults = async () => {
      try {
        ...
        toast.error(
          error.error || "Could not fetch the quiz results at this time",
        );
      } finally {
        setLoading(false);
      }
  ...
  const correctAnswers = detailedResults.reduce(
    (acc, currentValue) => (currentValue.isCorrect ? acc + 1 : acc),
    0,
  );
  ...
  return (
    <div className="max-w-5xl mx-auto">
      ...
      <PageHeader title={`${results.quiz.title || "Quiz"} Results`} />
      ...
      <div className="bg-white/80 backdrop-blur-xl border-2 border-slate-200 rounded-2xl shadow-xl shadow-slate-200/50 p-8 mb-8">
        <div className="text-center space-y-6">
          ...
          <div>
            ...
            <div
              className={`inline-block text-5xl font-bold border bg-linear-to-r ${getScoreColor(score)} bg-clip-text text-transparent mb-2 selection:bg-emerald-500 selection:text-white`}
            >
              {score}%
            </div>
            ...
            >
              ...
              <div className="space-y-3 mb-4">
                {result.options.map((option, optionIndex) => {
                  ...
                  const isUserAnswer = optionIndex === result.selectedAnswer; // selected by user
                  const isWrongAnswer = isUserAnswer && !isCorrect; // selected but not correct
                  return (
                    <div
                      key={optionIndex}
        ...
        <Link to={`/documents/${quiz.document._id}`}>
          <button className="group relative px-8 h-12 bg-linear-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 text-white font-semibold text-sm rounded-xl transition-all duration-200 shadow-lg shadow-emerald-500/25 active:scale-95 overflow-hidden">
            <span className="relative z-10 flex items-center gap-2">
              <ArrowLeft
                className="w-5 h-4 group-hover:-translate-x-1 transition-transform duration-200"
                strokeWidth={2}
              />
              Return to Document
            </span>
