...
const QuizTakePage = () => {
  ...
  useEffect(() => {
    const fetchQuiz = async () => {
      try {
        ...
        toast.error(error.error || "Could not fetch the quiz at this time");
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmitQuiz = async () => {
    setSubmitting(true);
    try {
      const formattedAnswers = Object.entries(selectedAnswers).reduce(
        ...
        <div className="space-y-3">
          {currentQuestion.options.map((option, index) => {
            ...
            return (
              <label
                key={index}
                className={`group relative flex p-3 border-2 rounded-xl cursor-pointer transition-all duration-200 ${isSelected ? "border-emerald-500 bg-emerald-50 shadow-lg shadow-emerald-500/10" : "border-slate-200 bg-slate-50/50 hover:border-slate-300 hover:bg-white hover:shadow-md"}`}
              >
                <input
                  type="radio"
          ...
          >
            <span className="relative z-10 flex items-center justify-center gap-2">
              {submitting ? (
                ...
                <>
                  <CheckCircle2 className="w-4 h-4" strokeWidth={2.5} /> Submit
                  Quiz
                </>
              )}
            </span>
